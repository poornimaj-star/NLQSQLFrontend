...
const Chat: React.FC<ChatProps> = ({ selectedAgent, onAgentChange }) => {
  ...
  const extractDatabasesFromSPResults = (spData: any[]): Database[] => {
    ...
    spData.forEach((config: any) => {
      ...
      const possibleSchemaFields = [
        'schema_name', 'schema', 'table_schema', 'default_schema'
      ];

      let dbName = null;
      let schemaName = "public"; // default schema
      ...
      // Find schema name
      for (const field of possibleSchemaFields) {
        if (config[field] && typeof config[field] === 'string') {
          schemaName = config[field];
          break;
        }
      }
  ...
  const loadDatabasesList = async () => {
    ...
    try {
      console.log("🔍 Loading databases for agent:", currentSelectedAgent.id);
      
      // Call the real API endpoint
      const response = await axios.get(`http://localhost:5000/api/databases/${currentSelectedAgent.id}`);
      console.log("📊 Database API response:", response.data);
      
      if (response.data?.success && Array.isArray(response.data.databases)) {
        const databaseList = response.data.databases.map((dbName: string) => ({
          name: dbName,
          schema: "public"
        }));
        ...
        if (databaseList.length > 0) {
          ...
          console.log("🎯 Auto-selected database:", databaseList[0].name);
        }
      } else {
  ...
  const handleAgentSelect = async (agent: Agent) => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/agent/${agent.id}/db-config`);
      console.log("📊 DB Config SP response:", response.data);
      
      if (response.data?.success && response.data.data && response.data.data.length > 0) {
        console.log(`✅ SP returned ${response.data.count} configuration rows`);
        console.log("📋 SP Result Structure Sample:", JSON.stringify(response.data.data[0], null, 2));
        
        // Extract database names from stored procedure results using helper function
        const databaseList = extractDatabasesFromSPResults(response.data.data);
        
        if (databaseList.length > 0) {
          console.log("🎯 Setting databases from SP results:", databaseList);
          setDatabases(databaseList);
          setSelectedDatabase(databaseList[0]);
          console.log("🎯 Auto-selected database from SP:", databaseList[0].name);
        } else {
          console.log("⚠️ No database names found in SP results, falling back to API");
          console.log("📋 Available fields in SP results:", Object.keys(response.data.data[0] || {}));
          loadDatabasesList();
        }
      } else {
    ...
    <div style={{ position: "relative", marginBottom: "10px" }}>
      <button
        ...
        style={{
          ...
          borderRadius: "4px",
          backgroundColor: databasesLoading ? "#f5f5f5" : "white",
          cursor: databasesLoading ? "not-allowed" : "pointer",
          display: "flex",
          justifyContent: "space-between",
          alignItems: "center",
          opacity: databasesLoading ? 0.7 : 1
        }}
      >
        <span>
          {databasesLoading 
        ...
        <span>{databasesLoading ? "⏳" : showDatabaseDropdown ? "▲" : "▼"}</span>
      </button>
